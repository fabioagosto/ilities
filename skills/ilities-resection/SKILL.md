...
description: >-
  Fix your position mid-build: take a quick bearing on where the change stands versus the intent
  it set out to serve, and report whether you are still on course or have drifted (and if
  drifted, where it started). Use this whenever you are partway through and unsure you are on
  track: "am I still on target", "have I gone off-course on this refactor", "does this still
  match what I set out to do", "I've been at this a while, check I haven't drifted". It sights
  known references: the stated intent (or North Star), the codebase's conventions, and what is
  done so far; crosses them, and returns a short position fix. This is a light, mid-flight
  check, not the full ilities review (the thorough, scored, merge-readiness audit) and not
  ilities-azimuth (which holds the line continuously as you write); reach for it as a discrete
  "where am I?" checkpoint during the work. Intent-first: it measures drift against the intent,
  so if the intent was never clear it says so and points you to pin it first.
---
