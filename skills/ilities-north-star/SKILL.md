...
description: >-
  Interrogate a fuzzy idea into a clear, checkable intent BEFORE any code is written: grill the
  user one pointed question at a time until the change's North Star (the win it steers toward,
  the one-sentence intent that serves it, and the scope boundaries) is explicit, then hand back
  a North Star brief. Use this whenever the intent is still vague: "help me figure out what I'm
  actually building", "grill me on this idea", "pressure-test this before I start", "what's the
  north star here", "am I even solving the right problem". This is the forward front of the
  suite: it runs before ilities-azimuth (which assumes a known intent and builds to it) and
  before ilities (which reviews a change that already exists). The distinguishing move: it
  refuses to let work start until the intent would pass the intent gate, catching a clean
  solution to the wrong problem before any code exists. Prefer this over jumping straight to
